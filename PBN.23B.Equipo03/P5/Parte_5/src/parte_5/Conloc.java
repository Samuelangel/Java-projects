...
public class Conloc {
        ...
        try {
            ...
            if (!(auxiliar.getSize().equals(" "))) {//analiza el valor de size, si no tiene valor es que es una linea con error
                ...
                if (auxiliar.getCodop().equals("ORG")) {//analiza si el CODOP es igual a ORG para seguir validando
                    ...
                    auxArchivo.writeBytes(Parte_5.validarDireccion(auxiliar.getOperando()) + ",       ");//escribe su valor en 2bytes en el .lst
                    auxiliar.setConloc(Parte_5.validarDireccion(auxiliar.getOperando()));//valida la direccion y la establece en conloc en 2bytes
                    ...
                    if (!(conloc.equals(" "))) {//valida que ya se haya inicializado el conloc
                        ...
                        auxArchivo.writeBytes(Parte_5.validarDireccion("$".concat(conloc)) + ",       ");//escribe en el .lst el nuevo valor del conloc en 2bytes
                        auxiliar.setConloc(Parte_5.validarDireccion("$".concat(conloc))); //guarda en el arreglo el valor del conloc en 2bytes de la linea correspondiente
    ...
    static void Creartabsim(){
        try {
            ...
            Parte_5.Errores.add("No se ha creado el archivo");//en caso de error no crea nada y manda aviso
        }
