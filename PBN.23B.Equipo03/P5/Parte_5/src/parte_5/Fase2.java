...
public class Fase2 {
    ...
    public static void postREL8(Linea relativo, String origen){
        ...
        if (!(destino.equals(" "))) {//si existe una direccion de memoria
            ...
            dest = Parte_5.ConvertirADecimal("$".concat(destino));//guarda el valor decimal del destino
    ...
    static String FormIMM(String opr, String sourceform, int size, int calcular) {
        ...
        Integer opraux = Parte_5.ConvertirADecimal(opr);                      //Convierte el operando a decimal para trabajar mejor con el
        if (opraux < 256 && size == 2 && calcular == 1 && frmbase[1].equals("ii")) {  //Verifica que sea un IMM de 8bits
            ...
            aux = Parte_5.validarDireccion(opr);
    ...
    static String FormDirExt(String opr, String sourceform,int size, int calcular){
        ...
        Integer opraux = Parte_5.ConvertirADecimal(opr); //convertimos a decimal
        if(opraux<256 && size==2 && calcular==1 && frmbase[1].equals("dd")){
            ...
            aux=Parte_5.validarDireccion(opr);//valida la direccion del opr y lo guarda en la variable aux
    ...
    static void fase2(){//Metodo para identificar rels y calcular su postbyte
        String direccion;
        for(Linea asm:Parte_5.LineasASM){ //for para recorrer todas las intrucciones
            switch (asm.getADDR()) {
            ...
            //CALCULO DEL POSTBYTE DE LOS IMM 
                case "IMM":
                    if (Parte_5.validarEtiq(asm.getOperando().substring(1))) {//Si el operando es una etiqueta
                        ...
                        if (direccion.equals(" ")) {//Si no existe la direccion
                            asm.setCop("ERROR");
                            Parte_5.Errores.add("ERROR etiqueta del operando no existe en " + asm.getCodop() + " " + asm.getOperando());
                ...
                case "DIR":
                    if (Parte_5.validarEtiq(asm.getOperando())) {
                        ...
                        if (direccion.equals(" ")) {//Si la etiqueta no existe
                            ...
                            if (asm.getADDR().equals("EXT")) {//Si era extendido y no directo 
                                int indiceObjetoActual = Parte_5.LineasASM.indexOf(asm);
